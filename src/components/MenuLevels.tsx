      ...
      {showCreateTextModal && (
        <div className={`fixed inset-0 ${isDarkMode ? 'bg-gray-900' : 'bg-black'} bg-opacity-50 flex items-center justify-center`}>
          <div className={`${isDarkMode ? 'bg-gray-800 text-white' : 'bg-white text-black'} p-4 rounded w-full max-w-lg`}>
            <h2 className={`text-xl font-bold mb-2 ${isDarkMode ? 'text-blue-300' : 'text-blue-600'}`}>
            ...
            <button
              ...
              className={`mt-4 px-4 py-2 ${isDarkMode ? 'bg-green-700 hover:bg-green-600' : 'bg-green-500 hover:bg-green-400'} text-white rounded`}
            >
              {t('menuLevels.modal.add')}
            </button>
            <button
              onClick={() => setShowCreateTextModal(false)}
              className={`mt-4 px-4 py-2 ${isDarkMode ? 'bg-blue-700 hover:bg-blue-600' : 'bg-blue-500 hover:bg-blue-400'} text-white rounded ml-2`}
            >
